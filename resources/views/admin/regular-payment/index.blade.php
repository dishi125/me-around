...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <a class="btn btn-primary" href="{{ route('admin.regular-payment.calendar-index') }}">Calendar</a>
                    @if($today_payments > 0)
                        <a class="btn btn-primary">Today: {{ $today_payments }}</a>
                    @endif
                    @if($missed_payments > 0)
                        <a class="btn btn-primary">Missed: {{ $missed_payments }}</a>
                    @endif
                    <a role="button" title="" data-original-title="" class="btn btn-primary" id="all-payments-user" data-toggle="tooltip">Activity Log</a>
...
<div class="modal fade" id="paymentLogModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"></div>

<div class="modal fade" id="allPaymentModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"></div>

@section('scripts')
    <script>
        var csrfToken = "{{ csrf_token() }}";
        var allTable = "{!! route('admin.regular-payment.table') !!}";
    </script>
    <script>
        $(function() {
        ...
        $(document).on('click', '#all-payments-user', function (){
            $.get(baseUrl + '/admin/show/all-payment', function (data, status) {
                $('#allPaymentModal').html('');
                $('#allPaymentModal').html(data);
                $('#allPaymentModal').modal('show');
            });
        })

        function updateNextPayDateVisibility(payment_id,is_hide_next_payment_date){
            $.ajax({
                method: 'POST',
                cache: false,
                data: { payment_id: payment_id, is_hide_next_payment_date: is_hide_next_payment_date },
                url: "{{ route('admin.regular-payment.next-payment-visibility') }}",
                success: function(results) {
                    $(".cover-spin").hide();
                    if(results.success == true) {
                        $('#regular-payment-table').DataTable().ajax.reload();
        ...
        $(document).on('click', '#removePaymentBtn', function (){
            ...
            $.ajax({
                ...
                url: "{{ route('admin.regular-payment.remove-payment') }}",
                success: function(results) {
                    $(".cover-spin").hide();
                    if(results.success == true) {
                        $("#editModal").modal('hide');
