        ...
        <div class="col-12 col-md-12 col-lg-12">
            <div class="card profile-widget">
                <div class="profile-widget-description">
                    <div class="">
                        @if ($isUpdate)
                            {!! Form::open(['route' => ['business.posts.update', $post->id], 'id' =>"postUpdateForm", 'method' => 'put', 'enctype' => 'multipart/form-data']) !!}
                        @else
                            {!! Form::open(['route' => 'business.posts.store', 'id' =>"postForm", 'enctype' => 'multipart/form-data']) !!}
                        @endif
                            @csrf
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-footer text-right">
                                    ...
                                    <a href="{{ route('business.posts.index')}}"
                                        class="btn btn-default">{{ __(Lang::get('general.cancel')) }}
                                    </a>
                                </div>
                            </div>
                        {!! Form::close() !!}
...
<script>
    ...
    $(document.body).on('submit','#postUpdateForm',function(event){
        ...
        $.ajax({
            url: "{{ route('business.posts.update',['post'=> $post->id]) }}",
            type: 'POST',
            contentType: false, 
            processData: false,
            data: formData,
    ...
    $(document.body).on('submit','#postForm',function(event){
        ...
        console.log(mainImagesFiles);
        $.ajax({
            url: "{{ route('business.posts.store') }}",
            type: 'POST',
            contentType: false, 
            processData: false,
            data: formData,
