<div class="row top-post-tabs">
               ...
               <div class="card p-0">
                  <div class="card-body p-0">
                     <ul class="nav nav-pills flex-column">
                        <li class="nav-item">
                           <a class="nav-link active show" data-toggle="list" href="#profile-list-normal-user-{{$countryData->id}}">Normal user</a>
                        </li>
                        <li class="nav-item">
                           <a class="nav-link" data-toggle="list" href="#profile-list-hopsital-{{$countryData->id}}">Hospital</a>
                        </li>

                        @foreach($beautyCategory as $key => $value)
                           <li class="nav-item">
                              <a class="nav-link" data-toggle="list" href="#profile-list-{{$countryData->id}}-{{$key}}">{{$value}}</a>
                           </li>
                        @endforeach                        
                     </ul>
                  </div>               
            ...
            <div class="col-8">               
               <div class="tab-content">
                  <div class="tab-pane fade active show" id="profile-list-normal-user-{{$countryData->id}}">
                     <div class="card">
                        <div class="card-header justify-content-between">
                           ...
                           <?php 
                              $checked = '';
                              if(count($normalUserSlides) > 0 && !empty($normalUserSlides) && $normalUserSlides->where('country_code',$countryData->code)->where('is_random', 1)->count() >= 1) {
                                 $checked = "checked";
                              }
                           ?>
                              <input type="checkbox" class="custom-control-input is-random-check" id="checkbox-normal-user" {{$checked}} data-cat-id="0" data-entity-id="{{ \App\Models\EntityTypes::NORMALUSER}}" data-section="profile">
                        ...
                        <div class="card-body">
                           <ul class="slide-show-list"> 
                              @if(count($normalUserSlides) > 0 && !empty($normalUserSlides))
                                 @if ($normalUserSlides->contains('country_code', $countryData->code))
                                    @foreach($normalUserSlides as $normalUserSlide )
                                       @if(!empty($normalUserSlide) && count($normalUserSlide->bannerDetail) > 0 && $normalUserSlide->country_code == $countryData->code)
                  ...
                  <div class="tab-pane fade" id="profile-list-hopsital-{{$countryData->id}}">
                     <div class="card">
                        <div class="card-header justify-content-between">
                           <h4>Profile slide</h4>
                           <div class="custom-checkbox custom-control">
                              <?php 
                                 $checked = '';
                                 if(count($hospitalUserSlides) > 0 && !empty($hospitalUserSlides) && $hospitalUserSlides->where('country_code',$countryData->code)->where('is_random', 1)->count() >= 1) {
                                    $checked = "checked";
                                 }
                              ?>
                              <input type="checkbox" class="custom-control-input is-random-check" id="checkbox-hospital" {{$checked}} data-cat-id="0" data-entity-id="{{ \App\Models\EntityTypes::HOSPITAL}}" data-section="profile">
                        ...
                        <div class="card-body">
                           ...
                           @if(count($hospitalUserSlides) > 0 && !empty($hospitalUserSlides))
                                 @if ($hospitalUserSlides->contains('country_code', $countryData->code))
                                    @foreach($hospitalUserSlides as $hospitalUserSlide )
                                       @if(!empty($hospitalUserSlide) && count($hospitalUserSlide->bannerDetail) > 0 && $hospitalUserSlide->country_code == $countryData->code)
                  ...
                  @foreach($beautyCategory as $key => $value)
                  <div class="tab-pane fade" id="profile-list-{{$countryData->id}}-{{$key}}">
                     <div class="card">
                        <div class="card-header justify-content-between">
                           <h4>Slide Show</h4>
                           <div class="custom-checkbox custom-control">
                           <?php 
                              $checked = '';
                              if(count($shopProfileSlides) > 0 && !empty($shopProfileSlides) && $shopProfileSlides->contains('category_id', $key) && $shopProfileSlides->where('category_id', $key)->where('country_code',$countryData->code)->where('is_random', 1)->count() >= 1) {
                                 $checked = "checked";
                              }
                           ?>
                              <input type="checkbox" class="custom-control-input is-random-check" id="checkbox-{{$key}}" {{$checked}} data-cat-id="{{ $key }}" data-entity-id="{{ \App\Models\EntityTypes::SHOP}}" data-section="profile">
                        ...
                        <div class="card-body">
                           <ul class="slide-show-list">
                              @if(count($shopProfileSlides) > 0 && !empty($shopProfileSlides))
                                 @if ($shopProfileSlides->contains('category_id', $key) && $shopProfileSlides->contains('country_code', $countryData->code))
                                    @foreach($shopProfileSlides as $shopProfileSlide )
                                       @if(!empty($shopProfileSlide) && $shopProfileSlide->category_id == $key && count($shopProfileSlide->bannerDetail) > 0 && $shopProfileSlide->country_code == $countryData->code)
