                    ...
                    <select class="form-control" id="select_report_category" name="select_report_category">
                        <option value="0">{!! __(Lang::get('forms.report-client.select')) !!}</option>                    
                            @foreach($categoryList as $category)
                                @if(!empty($category))
                                    @if($category->parent_name == 'Hospital' || $category->parent_name == 'Shop' || $category->parent_name == 'Community' || $category->parent_name == 'Shop User')
                                    <option                                
...
@section('scripts')
<script>
var hospitalIndex = "{!! route('admin.reported-client.all.table') !!}";
...
<script src="{!! asset('js/pages/reported-client/all-report.js') !!}"></script>
