...
$coin_amount= !empty($data->coin_amount) ? $data->coin_amount : '' ;
                ...
                <div class="profile-widget-description">
                    <div class="">
                        @if ( isset($brand_product ))
                            {!! Form::open(['route' => ['admin.brand-products.update', $id], 'id' =>"categoryUpdateForm", 'method' => 'put', 'enctype' => 'multipart/form-data']) !!}
                        @else
                            {!! Form::open(['route' => 'admin.brand-products.store', 'id' =>"categoryForm", 'enctype' => 'multipart/form-data']) !!}
                        @endif
                            @csrf
                            ...
                            <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            {!! Form::label('brand_id', __(Lang::get('forms.brands.brand'))); !!}
                                            {!!Form::select('brand_id', $brands, $brand_id , ['class' => 'form-control select2','placeholder' => __(Lang::get('forms.brands.brand'))])!!}
                                        </div>
                                    </div>

                                    <div class="col-md-4">
                                        <div class="form-group">
                                            {!! Form::label('coin_amount', __(Lang::get('forms.brands.coin_amount'))); !!}
                                            {!! Form::text('coin_amount', $coin_amount, ['class' => 'numeric form-control', 'placeholder' => __(Lang::get('forms.brands.coin_amount')) ]); !!}
                                        </div>
                                    </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {!! Form::label('product_image', __('Product Image')); !!}
                                        <div class="d-flex align-items-center">
                                            <div id="image_preview" class="d-flex">
                                                @if($id)
                                                    <?php
                                                        $imageURL = $brand_product->product_image ? Storage::disk('s3')->url($brand_product->product_image) : '';
                                        ...
                                        <div class="add-image-icon" style="display: {{($imageURL) ? 'none' : 'flex'}};" >
                                            {{ Form::file('product_image',[ 'accept'=>"image/jpg,image/png,image/jpeg", 'onchange'=>"imagesPreview(this, '#image_preview');", 'class' => 'main_image_file form-control', 'id' => "product_image", 'hidden' => 'hidden' ]) }}
                                            <label class="pointer" for="product_image"><i class="fa fa-plus fa-4x"></i></label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer text-right">
                                ...
                                <a href="{{ route('admin.brand-products.index')}}"
    ...
    if (input.files) {
        ...
        Array.from(input.files).forEach(async (file,index) => { 
            ...
            reader.onload = function(event) {
                ...
                var container = jQuery("<div></div>",{class: "removeImage", html:'<span fieldname="product_image" data-timestemp="'+currentTimestemp+'" class="pointer"><i class="fa fa-times-circle fa-2x"></i></span>'});
